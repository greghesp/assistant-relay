...
function activatePortForwarding(minIp, port) {
  return new Promise((resolve) => {
    if (process.platform === "win32") {
      exec(
        `netsh interface portproxy add v4tov4 listenport=80 listenaddress=${minIp} connectport=${port} connectaddress=127.0.0.1`,
        {},
        (err) => {
          if (err) {
            error(err);
      ...
      exec(
        `echo "rdr pass on lo0 inet proto tcp from any to ${minIp} port 80 -> 127.0.0.1 port ${port}" | sudo pfctl -ef -`,
        {},
        () => {
          resolve();
        }
...
function removeFromEtcHosts(url) {
  return new Promise((resolve) => {
    ...
    function filterFunc(line) {
      return !(
        (Array.isArray(line) && line[1] === url) ||
        (typeof line === "string" && line.split(" ")[2] === this.port)
      );
    }
