...
for(var user in authKeys) {
  if (authKeys.hasOwnProperty(user)) {
    config.users[`${user}`] = {}

    config.users[`${user}`].keyFilePath = path.resolve(__dirname, authKeys[user]);
    config.users[`${user}`].savedTokensPath = path.resolve(__dirname,`${user}-tokens.json`);
  }
}

var usersX = config.users;

for(var assistant in usersX) {
  const `${usersX}` = new GoogleAssistant(config.users[`${usersX}`]);
}
