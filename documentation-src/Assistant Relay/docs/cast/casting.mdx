...
> If you want to cast a local file on your system that is outside of the `/bin/media` folder. You can pass the system path to the file as the source using the below body
...
> Local files must be placed into the `/bin/media` folder of Assistant Relay.  If you want to cast a local file that is not within the Assistant Relay directory, use the type `remote` and pass the system path of the file as the source
...
At the moment, this is very much a *work in progress*, and to sample this, we have added a simple doorbell sound effect
...
> **NOTE**:  There is a bug with Smart Speakers and Displays where the cast session stays in progress after local media has been played.  This can be seen by the black screen with the cast icon inside it
